...
def main2(encuesta,year, month,nomencuesta):
    ...
    # query_mysql = f"SELECT token,attribute_4,attribute_24,attribute_7,attribute_8,attribute_9,attribute_11,attribute_12,attribute_13 FROM lime_tokens_{encuesta} "    
    ...
    # con.execute("""CREATE TABLE mysql_table AS SELECT * FROM read_csv_auto('tabla_mysql.csv', delim=',', quote='"');""")
    # # con.execute("CREATE TABLE contestados AS SELECT * FROM read_csv_auto('tabla_mysql_contestados.csv');")

    

    # # con.execute("CREATE TABLE eliminar AS SELECT mysql_table.token FROM mysql_table LEFT JOIN pg_table ON mysql_table.token = pg_table.token WHERE pg_table.token IS NULL;")
    # # Leer la tabla desde DuckDB Los que se deben eliminar
    # # df_to_delete = con.execute("SELECT mysql_table.token FROM mysql_table LEFT JOIN pg_table ON mysql_table.token = pg_table.token WHERE pg_table.token IS NULL;").fetchdf()
    ...
    # # Insertar en la tabal de respaldo todos los movimiento realizados , Los nuevos , los con ID_RBD diferentes y los eliminados
    # movimientos = f"""
    # select
    # 'nuevos' tipo,
    # p.token,
    ...
    # p.attribute_11 Region,
    # p.attribute_13 Comuna,
    # {yearmonth} fechaSige,
