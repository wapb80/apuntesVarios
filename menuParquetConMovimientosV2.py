...
def main2(encuesta,year, month,nomencuesta):
    ...
    mysql_engine_test = create_engine('mysql+pymysql://lm_desa:lm_desa_Sead2023@192.168.8.2:3306/test')
    ...
    query_mysql = f"SELECT * FROM lime_tokens_{encuesta}_prueba "
    ...
    if not df_to_update.empty:
                  for index, row in df_to_update.iterrows():
                    sql = f"""
                    UPDATE lime_tokens_{encuesta}_prueba SET completed = '{row['completed']}' WHERE token = '{row['token']}';
                    """
                    # print(sql)
                    with mysql_engine.connect() as connection:
    ...
    #################################################################################################
    # Insertar en la tabLA de respaldo todos los movimiento realizados , 
    # Los nuevos , los con ID_RBD diferentes y los eliminados
    # movimientos = f"""
    # select
    # 'Nuevos' tipo,
    # p.token,
    # p.attribute_4 nuevo_id_RBD,
    # m.attribute_4  antiguo_id_RBD,
    # p.attribute_24 nuevo_RBD,
    # m.attribute_24 antiguo_RBD,
    # p.attribute_7 nueva_Ensenanza,
    # m.attribute_7 antiguo_Ensenanza,
    # p.attribute_9 nuevo_Curso,
    # m.attribute_9 antiguo_Curso,
    # p.attribute_8 ID_grado_curso,
    # p.attribute_11 Nueva_Region,
    # p.attribute_13 Nueva_Comuna,
    # m.attribute_11 Antigua_Region,
    # m.attribute_13 Antigua_Comuna, 
    # {yearmonth} fechaSige,
    # '{nomencuesta}' encuesta,
    # CAST(strftime(now(),'%Y%m%d%H%M' ) AS BIGINT) fechaIngreso
    # from pg_table as p
    # left join mysql_table as m on p.token = m.token
    ...
    # '{nomencuesta}' encuesta,
    # CAST(strftime(now(),'%Y%m%d%H%M' ) AS BIGINT) fechaIngreso
    # from mysql_table as m
    # left join pg_table as p on p.token = m.token and p.attribute_4 != m.attribute_4
    ...
    # CAST(strftime(now(),'%Y%m%d%H%M' ) AS BIGINT) fechaIngreso
    # from mysql_table m
    # left join pg_table p on m.token = p.token
    # where p.token is null
    # ;
    # """
    # print("Grabando Movimientos")
    # # Ejecutar la consulta y guardar el resultado en un DataFrame
    # df_movimientos = con.execute(movimientos).fetchdf()
    # # Insertar los datos en la tabla de MySQL
    # tabla="movimientos"
    # # print(movimientos)
    # df_movimientos.to_sql(tabla, mysql_engine_test, if_exists='append', index=False)
    
### hasta aca para hacer pruebas 
    #################################################################################################
    #################################################################################################    
    #################################### DESDE ACA LA NUEVA FUNCIONALIDAD DE GRABAR UNA COPIA DE LOS REGUISTROS
    #                                                       ELIMINADOS Y MODIFICADOS
    
    
    # Leer la tabla desde DuckDB Los que se deben eliminar
    # df_to_delete2 = con.execute("SELECT mysql_table.token FROM mysql_table LEFT JOIN pg_table ON mysql_table.token = pg_table.token WHERE pg_table.token IS NULL ;").fetchdf()
    #  # Formatear las claves en un string separado por comas
    # claves = ','.join(map(str, df_to_delete2['token'].tolist()))
    # # Construir la segunda consulta en el sige con el listado anterior
   
    # la siguiente es solo para los que se eliminaron y tienen la encuesta completa
    # query_insertEliminados = f""" SELECT * from mysql_table WHERE completed !='N' and token  IN ({claves}) """
    
    # todas las encuestas que se elimnaron
    query_insertEliminados = f"""SELECT mysql_table.token FROM mysql_table LEFT JOIN pg_table ON mysql_table.token = pg_table.token WHERE pg_table.token IS NULL ;"""
    
    # print(query_insertEliminados)     
               
    df_insertEliminados = con.execute(query_insertEliminados).fetchdf()
    # Insertar los datos en la tabla de MySQL
    tablainsertEliminados=f"lime_tokens_{encuesta}_prueba"
 
    df_insertEliminados.to_sql(tablainsertEliminados, mysql_engine_test, if_exists='append', index=False)
    
    
    # ################################ VER CUALES SE MOVIERON Y ESTABAN CON LA ENCUESTA CONTESTADA
    # df_to_update2 = con.execute("select  m.token  from mysql_table as m left Join pg_table as p ON p.token=m.token and p.attribute_4!=m.attribute_4 where p.token is not null and m.completed!='N' ").fetchdf()
     # Formatear las claves en un string separado por comas
    # claves2 = ','.join(map(str, df_to_update2['token'].tolist()))
    # Construir la segunda consulta en el sige con el listado anterior
    # Leer tabla de PostgreSQL
    # query_insertModificados = f"""SELECT * from mysql_table WHERE token  IN ({claves2})"""
    query_insertModificados = f"""select  m.token  from mysql_table as m left Join pg_table as p ON p.token=m.token and p.attribute_4!=m.attribute_4 where p.token is not null and m.completed!='N' """
   
    df_insertModificado = con.execute(query_insertModificados).fetchdf()
    # Insertar los datos en la tabla de MySQL
    # tablainsertModificados="lime_tokens_{encuesta}_prueba"
    tablainsertModificados=f"lime_tokens_{encuesta}_prueba"
    # print(movimientos)
    df_insertModificado.to_sql(tablainsertModificados, mysql_engine_test, if_exists='append', index=False)
    #################################################################################################
    #################################################################################################
