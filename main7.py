...
SELECT p.* FROM pg_table as p 
LEFT JOIN mysql_table as m  ON p.token = m.token 
WHERE m.token IS NULL   
AND p.token NOT IN (SELECT token FROM mysql_table_contestados );
"""
...
query_eliminar = """
SELECT mysql_table.token
FROM mysql_table
LEFT JOIN pg_table
ON mysql_table.token = pg_table.token
WHERE pg_table.token IS NULL
AND mysql_table.token NOT IN (SELECT token FROM mysql_table_contestados );
"""
...
# # Saber los que hay que eliminar de los movidos , mientras no se alla contestado
# query_eliminar = """
